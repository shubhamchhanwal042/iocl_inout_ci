...
class DriverLicenseModel extends CI_Model {
    ...
    public function UpdateStatus($uniqueValue) {
        ...
        $query = $this->db->get('driverlicensegate', 1); // Limit the query to just the latest record
        ...
        if ($query->num_rows() > 0) {
            ...
            // print_r('id');die;
            $this->db->update('driverlicensegate', ['status' => $newStatus, 'outtime' => $outtime]);
    ...
    // print_r($data);die; 
        ...
        $this->db->insert('driverlicensegate', $data);
