...
class LicenseGateModel extends CI_Model {
    ...
    public function Dashboard()
	{
		...
		$this->load->view('gates/licensegate', $data);
	}

    public function EnterLicengate($uniqueValue, $table) {
    ...
    public function UpdateStatus($uniqueValue) {
        ...
        $query = $this->db->get('licensegate', 1); // Limit the query to just the latest record
        ...
        if ($query->num_rows() > 0) {
            ...
            // print_r('id');die;
            $this->db->update('licensegate', ['status' => $newStatus, 'outtime' => $outtime]);
    ...
    public function InsertLicensgate($uniqueValue, $records, $CheckMaingate) {
    ...
    // print_r($data);die; 
        ...
        $this->db->insert('licensegate', $data);
    ...
    function CheckLicengateStatus($uniqueValue) {
        ...
        if ($query->num_rows() > 0) {
            // Return the full details of the most recent entry
            return $query->row();  // This will return the entire row as an object
        } else {
            // No result found for the provided QR code
            return null;  // Return null or any other message indicating no data
        }
    }
}
?>