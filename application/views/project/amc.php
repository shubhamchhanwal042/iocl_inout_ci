...
<script>
    ...
    <div class="wrapper d-flex overall-body">
                            ...
                            <div id="officerForm" class="row">
                                ...
                                <div class="col-lg-5">
                                    <input type="text" name="addtoken"
                                        ...
                                        maxlength="2" id="amcinput" class="form-control" placeholder="Enter Number of Cylinders">
                                    <!-- addcard -->
                                </div>
                                <!-- change button -->
                                <div class="col-lg-7">
        ...
        <script>
        // Fetch amc data and generate cards
        function fetchAmc() {
        fetch("<?php echo base_url('Project/FetchAmc'); ?>")
                    ...
                    card.innerHTML = `
                        <div class="card text-center" id="${uniqueCardId}">
                            ...
                            <div class="card-body">
                                ...
                                <a href="${result.full_name ? 'EditAmc/' + result.id : 'AddAmc/' + result.id}" class="btn btn-primary form-control ${buttonDisabled}">
    ...
    document.getElementById('amcform').addEventListener('submit', function(e) {
        e.preventDefault();
        let tokenValue = document.getElementById('amcinput').value;
        if (tokenValue.trim() === '') {
            alert('Please enter a valid token.');
            return;
        }

        fetch("<?php echo site_url('Project/AddAmcToken'); ?>", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
            ...
            .then(response => response.json())
            .then(data => {
                fetchAmc(); // Re-fetch officers after adding
            });
    });

    // Initial fetch of packed on page load
    document.addEventListener('DOMContentLoaded', fetchAmc);
