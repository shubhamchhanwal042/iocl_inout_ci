...
<!-- ++++++++++++++++++++++++++++++++++++++++++++++DASHBOARD CONTENT END HERE+++++++++++++++++++++++++++++++++++ -->
          ...
          <script>
               ...
               document.getElementById('navbar-title').innerHTML = "GAT <i class='fas fa-users'></i>";
          </script>
          ...
          <script>
                    // Fetch GAT data and generate cards
                    function fetchGATs() {
                         fetch("<?php echo base_url('Operation/FetchGATs'); ?>")
                              ...
                              .then(data => {
                                   ...
                                   data.forEach(result => {
                                        ...
                                        // Check if GAT is already in the plant
                                        let gatStatusMessage = "";
                                        let buttonDisabled = "";
                                        if ( result.mg_data !== null && result.mg_data.status === '1') { // Officer is already in plant
                                             gatStatusMessage = "Already In Plant";
                                             ...
                                             <div class="card text-center" id="${uniqueCardId}">
                                                  <div class="error-box w-100 d-block text-start">
                                                       <p class="align-center mb-0 ps-1 pe-1 fs-5 text-center">&nbsp;
                                                            <span class="bg-danger text-light">${gatStatusMessage}</span>
                                                       </p>
                                                  </div>
                                                  ...
                                                  <div class="card-body">
                                                       ...
                                                       <a href="${result.full_name ? 'EditGat/' + result.id : 'AddGat/' + result.id}" class="btn btn-primary form-control ${buttonDisabled}">
                    ...
                    document.getElementById('gatForm').addEventListener('submit', function(e) {
                         e.preventDefault();
                         let tokenValue = document.getElementById('gatInput').value;
                         if (tokenValue.trim() === '') {
                              alert('Please enter a valid token.');
                              return;
                         }

                         fetch("<?php echo site_url('Operation/AddGATToken'); ?>", {
                                   method: 'POST',
                                   headers: {
                                        'Content-Type': 'application/x-www-form-urlencoded',
                                   },
                              ...
                              .then(response => response.json())
                              .then(data => {
                                   fetchGATs(); // Re-fetch GATs after adding
                              });
                    });

                    // Initial fetch of GATs on page load
                    document.addEventListener('DOMContentLoaded', fetchGATs);
          </script>
         
         </script>
             <script>
  document.getElementById('gatForm').addEventListener('submit', function(event) {
