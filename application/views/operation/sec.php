    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        <?php } ?>
          ...
          <!-- Custom JavaScript -->
           ...
           <script>
                            // Fetch SEC data and generate cards
          function fetchSEC() {
          fetch("<?php echo base_url('Operation/FetchSec'); ?>")
                              ...
                              <div class="card text-center" id="${uniqueCardId}">
                                   <div class="error-box w-100 d-block text-start">
                                        <p class="align-center mb-0 ps-1 pe-1 fs-5 text-center">&nbsp;
                                             <span class="bg-danger text-light">${secStatusMessage}</span>
                                        </p>
                                   </div>
                                   ...
                                   <div class="card-body">
                                        ...
                                        <a href="${result.full_name ? 'EditSec/' + result.id : 'AddSec/' + result.id}" class="btn btn-primary form-control ${buttonDisabled}">
     ...
     document.getElementById('secForm').addEventListener('submit', function(e) {
          e.preventDefault();
          let tokenValue = document.getElementById('secInput').value;
          if (tokenValue.trim() === '') {
               alert('Please enter a valid token.');
               return;
          }

          fetch("<?php echo site_url('Operation/AddSecToken'); ?>", {
                    method: 'POST',
                    headers: {
                         'Content-Type': 'application/x-www-form-urlencoded',
                    },
               ...
               .then(response => response.json())
               .then(data => {
                    fetchSEC(); // Re-fetch SEC after adding
               });
     });

     // Initial fetch of SEC on page load
     document.addEventListener('DOMContentLoaded', fetchSEC);
            </script>
          <script>
