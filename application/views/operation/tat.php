...
<!-- ++++++++++++++++++++++++++++++++++++++++++++++DASHBOARD CONTENT END HERE+++++++++++++++++++++++++++++++++++ -->
          ...
          <!-- Custom JavaScript -->
            ...
            <script>
                            // Fetch TAT data and generate cards
          function fetchTATs() {
          fetch("<?php echo base_url('Operation/FetchTATs'); ?>")
               ...
               .then(data => {
                    ...
                    data.forEach(result => {
                         ...
                         // Check if TAT is already in the plant
                         let tatStatusMessage = "";
                         let buttonDisabled = "";
                         if ( result.mg_data !== null && result.mg_data.status === '1') { // Officer is already in plant
                              tatStatusMessage = "Already In Plant";
     ...
     document.getElementById('tatForm').addEventListener('submit', function(e) {
          e.preventDefault();
          let tokenValue = document.getElementById('tatInput').value;
          if (tokenValue.trim() === '') {
               alert('Please enter a valid token.');
               return;
          }

          fetch("<?php echo site_url('Operation/AddTATToken'); ?>", {
                    method: 'POST',
                    headers: {
                         'Content-Type': 'application/x-www-form-urlencoded',
                    },
               ...
               .then(response => response.json())
               .then(data => {
                    fetchTATs(); // Re-fetch TATs after adding
               });
     });

     // Initial fetch of TATs on page load
     document.addEventListener('DOMContentLoaded', fetchTATs);
          </script>
     <script>
  document.getElementById('tatForm').addEventListener('submit', function(event) {
