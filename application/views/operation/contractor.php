...
<!-- ++++++++++++++++++++++++++++++++++++++++++++++DASHBOARD CONTENT END HERE+++++++++++++++++++++++++++++++++++ -->
        ...
        // Fetch contractor data and generate cards
        function fetchContractors() {
        fetch("<?php echo base_url('Operation/FetchContractors'); ?>")
            ...
            data.forEach(result => {
                ...
                // Check if contractor is already in the plant
                let contractorStatusMessage = "";
                let buttonDisabled = "";
                if ( result.mg_data !== null && result.mg_data.status === '1') { // Officer is already in plant
                    contractorStatusMessage = "Already In Plant";
                ...
                <div class="card text-center" id="${uniqueCardId}">
                    ...
                    <p class="align-center mb-0 ps-1 pe-1 fs-5 text-center">&nbsp;
                        <span class="bg-danger text-light">${contractorStatusMessage}</span>
                    </p>
                    </div>
                    ...
                    <a href="${result.full_name ? 'EditContractor/' + result.id : 'AddContractor/' + result.id}" class="btn btn-primary form-control ${buttonDisabled}">
        ...
        e.preventDefault();
        let tokenValue = document.getElementById('contractorInput').value;
        if (tokenValue.trim() === '') {
            alert('Please enter a valid token.');
            return;
        }

        fetch("<?php echo site_url('Operation/AddContractorToken'); ?>", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
            ...
            .then(response => response.json())
            .then(data => {
            fetchContractors(); // Re-fetch contractors after adding
            });
        });

        // Initial fetch of contractors on page load
        document.addEventListener('DOMContentLoaded', fetchContractors);
