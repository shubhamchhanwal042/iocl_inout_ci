...
<!-- ++++++++++++++++++++++++++++++++++++++++++++++DASHBOARD CONTENT END HERE+++++++++++++++++++++++++++++++++++ -->
          ...
          <!-- Custom JavaScript -->
            ...
            <script>
                            // Fetch FEG data and generate cards
          function fetchFEGs() {
          fetch("<?php echo base_url('Operation/FetchFEGs'); ?>")
               ...
               .then(data => {
                    ...
                    data.forEach(result => {
                         ...
                         // Check if FEG is already in the plant
                         let fegStatusMessage = "";
                         let buttonDisabled = "";
                         if ( result.mg_data !== null && result.mg_data.status === '1') { // Officer is already in plant
                              fegStatusMessage = "Already In Plant";
                              ...
                              <div class="card text-center" id="${uniqueCardId}">
                                   <div class="error-box w-100 d-block text-start">
                                        <p class="align-center mb-0 ps-1 pe-1 fs-5 text-center">&nbsp;
                                             <span class="bg-danger text-light">${fegStatusMessage}</span>
                                        </p>
                                   </div>
                                   ...
                                   <div class="card-body">
                                        ...
                                        <a href="${result.full_name ? 'EditFeg/' + result.id : 'AddFeg/' + result.id}" class="btn btn-primary form-control ${buttonDisabled}">
     ...
     document.getElementById('fegForm').addEventListener('submit', function(e) {
          e.preventDefault();
          let tokenValue = document.getElementById('fegInput').value;
          if (tokenValue.trim() === '') {
               alert('Please enter a valid token.');
               return;
          }

          fetch("<?php echo site_url('Operation/AddFEGToken'); ?>", {
                    method: 'POST',
                    headers: {
                         'Content-Type': 'application/x-www-form-urlencoded',
                    },
               ...
               .then(response => response.json())
               .then(data => {
                    fetchFEGs(); // Re-fetch FEGs after adding
               });
     });

     // Initial fetch of FEGs on page load
     document.addEventListener('DOMContentLoaded', fetchFEGs);
          </script>

<script>
  document.getElementById('fegForm').addEventListener('submit', function(event) {
