...
<body>
          ...
          <div class="container-fluid">
               ...
               <div class="container-fluid">
                    ...
                    <div class="counter-div">
                         ...
                         <form id="contractor_workmanForm">
                              <div class="row">
                                   <div class="col-lg-5">
                                        <label for="" class="fs-3">Mathadi Cylinder Count</label>
                                   </div>
                                   <div class="col-lg-7">
                              ...
                              <div id="contractor_workmanform" class="row">
                                   ...
                                   <div class="col-lg-5">
                                        <input type="text" name="addtoken"
                                             ...
                                             minlength="2" maxlength="2" id="contractor_workmanInput"
                                             class="form-control">
                                   </div>
                                   <!-- change button -->
                                   <div class="col-lg-7">
    ...
    document.addEventListener('DOMContentLoaded', function() {
     ...
     <script>
          document.getElementById('page-title').innerHTML = "IOCL  INOUT | Mathadi";
          document.getElementById('navbar-title').innerHTML = "Mathadi <i class='fa-solid fa-users'></i>";
     </script>
     ...
     <script>
          // Fetch contractor workman data and generate cards
          function fetchMathadi() {
               fetch("<?php echo base_url('Operation/FetchMathadi'); ?>")
                    .then(response => response.json())
                    .then(data => {
                         let cardContainer = document.getElementById('cardContainer');
                         cardContainer.innerHTML = ''; // Clear previous content

                         data.forEach(mathadi => {
                              let card = document.createElement('div');
                              card.classList.add('col-sm-6', 'col-md-3', 'col-lg-2', 'mb-5');
                              let uniqueCardId = `card-${mathadi.token_no}`;

                              // Check if workman is already in the plant
                              let workmanStatusMessage = "";
                              let buttonDisabled = "";
                              if (mathadi.status === '') { // Workman is already in plant
                                   workmanStatusMessage = "Already In Plant";
                                   ...
                                   <div class="card text-center" id="${uniqueCardId}">
                                        <div class="error-box w-100 d-block text-start">
                                             <p class="align-center mb-0 ps-1 pe-1 fs-5 text-center">&nbsp;
                                                  <span class="bg-danger text-light">${workmanStatusMessage}</span>
                                             </p>
                                        </div>
                                        ...
                                        <div class="card-body">
                                             <h5 class="card-title" style="height: 30px; overflow: hidden;">${mathadi.full_name || '<?php null; ?>'}</h5>
                                             <a href="${mathadi.full_name ? 'EditMathadi/' + mathadi.id : 'AddMathadi/' + mathadi.id}" class="btn btn-primary form-control ${buttonDisabled}">
                                                  <i class="fa-solid fa-${mathadi.full_name ? 'edit' : 'plus'}"></i>
                                                  ${mathadi.full_name ? 'Edit' : 'Add'}
                                             </a>
                                        </div>
                                   </div> 
                              `;
          ...
          document.getElementById('contractor_workmanForm').addEventListener('submit', function (e) {
               e.preventDefault();
               let tokenValue = document.getElementById('contractor_workmanInput').value;
               if (tokenValue.trim() === '') {
                    alert('Please enter a valid token.');
                    return;
               }

               fetch("<?php echo site_url('Operation/AddMathadiToken'); ?>", {
                    method: 'POST',
                    headers: {
                    ...
                    .then(response => response.json())
                    .then(data => {
                         fetchMathadi(); // Re-fetch workmen after adding
                    });
          });

          // Initial fetch of workmen on page load
          document.addEventListener('DOMContentLoaded', fetchMathadi);
     </script>
     </script>
             <script>
  document.getElementById('contractor_workmanForm').addEventListener('submit', function(event) {
