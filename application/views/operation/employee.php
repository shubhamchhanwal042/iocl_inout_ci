                    ...
                    <div id="officerForm" class="row">
                        ...
                        <div class="col-lg-5">
                            <input type="text" name="addtoken" oninput="this.value=this.value.replace(/[^0-9]/g,'')"
                                size="2" minlength="2" maxlength="2" id="employeeInput" class="form-control">
                            <!-- addcard -->
                        </div>
                        <!-- change button -->
                        <div class="col-lg-7">
...
<!-- 
 ...
 -->
          ...
          <script>
               document.getElementById('page-title').innerHTML = "IOCL  INOUT | Employee";
...
<script>
// Fetch employee data and generate cards
function fetchEmployees() {
     fetch("<?php echo base_url('Operation/FetchEmployees'); ?>")
                         ...
                         <div class="card text-center" id="${uniqueCardId}">
                              <div class="error-box w-100 d-block text-start">
                                   <p class="align-center mb-0 ps-1 pe-1 fs-5 text-center">&nbsp;
                                        <span class="bg-danger text-light">${employeeStatusMessage}</span>
                                   </p>
                              </div>
                              ...
                              <div class="card-body">
                                   ...
                                   <a href="${result.full_name ? 'EditEmployee/' + result.id : 'AddEmployee/' + result.id}" class="btn btn-primary form-control ${buttonDisabled}">
...
document.getElementById('employeeForm').addEventListener('submit', function(e) {
     e.preventDefault();
     let tokenValue = document.getElementById('employeeInput').value;
     if (tokenValue.trim() === '') {
          alert('Please enter a valid token.');
          return;
     }

     fetch("<?php echo site_url('Operation/AddEmployeeToken'); ?>", {
               method: 'POST',
               headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
               },
          ...
          .then(response => response.json())
          .then(data => {
               fetchEmployees(); // Re-fetch employees after adding
          });
});

// Initial fetch of employees on page load
