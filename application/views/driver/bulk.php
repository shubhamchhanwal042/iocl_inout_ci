...
<script>
        ...
        // Fetch bulk data and generate cards
        function fetchBulk() {
        fetch("<?php echo base_url('Driver/FetchBulk'); ?>")
            ...
            .then(data => {
                ...
                data.forEach(result => {
                    ...
                    // Check if bulk is already in the plant
                    let bulkStatusMessage = "";
                    let buttonDisabled = "";
                    if(result.mg_data !== null && result.mg_data.status === '1') { // bulk is already in plant
                        bulkStatusMessage = "Already In Plant";
                        ...
                        <div class="card text-center" id="${uniqueCardId}">
                            <div class="error-box w-100 d-block text-start">
                                <p class="align-center mb-0 ps-1 pe-1 fs-5 text-center">&nbsp;
                                    <span class="bg-danger text-light">${bulkStatusMessage}</span> 
                                </p>
                            </div>
                            ...
                            <div class="card-body">
                                ...
                                <a href="${result.full_name ? 'EditBulk/' + result.id : 'AddBulk/' + result.id}" class="btn btn-primary form-control ${buttonDisabled}">
    ...
    document.getElementById('bulkForm').addEventListener('submit', function(e) {
        e.preventDefault();
        let tokenValue = document.getElementById('bulkInput').value;
        if (tokenValue.trim() === '') {
            alert('Please enter a valid token.');
            return;
        }

        fetch("<?php echo site_url('Driver/AddBulkToken'); ?>", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
            ...
            .then(response => response.json())
            .then(data => {
                fetchBulk(); // Re-fetch officers after adding
            });
    });

    // Initial fetch of bulk on page load
    document.addEventListener('DOMContentLoaded', fetchBulk);
