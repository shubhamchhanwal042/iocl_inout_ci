...
<script>
        ...
        fetch("<?php echo base_url('Driver/FetchTransporter'); ?>")
            ...
            .then(data => {
                ...
                data.forEach(result => {
                    ...
                    let buttonDisabled = "";
                    if (result.mg_data !== null && result.mg_data.status === '1') { // transporter is already in plant
                    ...
                    card.innerHTML = `
                        <div class="card text-center" id="${uniqueCardId}">
                            ...
                            <div class="card-body">
                                ...
                                <a href="${result.full_name ? 'EditTransporter/' + result.id : 'AddTransporter/' + result.id}" class="btn btn-primary form-control ${buttonDisabled}">
    ...
    document.getElementById('transporterForm').addEventListener('submit', function(e) {
        e.preventDefault();
        let tokenValue = document.getElementById('transporterInput').value;
        if (tokenValue.trim() === '') {
            alert('Please enter a valid token.');
            return;
        }

        fetch("<?php echo base_url('Driver/AddTransporterToken'); ?>", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
            ...
            .then(response => response.json())
            .then(data => {
                fetchTransporter(); // Re-fetch officers after adding
            });
    });

    // Initial fetch of bulk on page load
    document.addEventListener('DOMContentLoaded', fetchTransporter);
